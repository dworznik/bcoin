...
function ChainDB(chain, options) {
  ...
  this.highest = network.genesis.height - 1;
  this.tip = null;
  this.height = -1;
  this.size = 0;
  this.fd = null;
...
ChainDB.prototype._init = function _init() {
  var genesis = bcoin.chainblock.fromJSON(this.chain, {
    ...
    height: network.genesis.height
  });

  if (!bcoin.fs) {
    utils.debug('`fs` module not available. Falling back to ramdisk.');
    if(this.data) {
      utils.debug('Init ramdisk data, size: ' + this.data.length);
      this.ramdisk = new bcoin.ramdisk(this.data, 40 * 1024 * 1024);
    } else {
  ...
  if (this.size === 0) {
    ...
    this.getSync(network.genesis.height);
...
ChainDB.prototype.load = function load(start, callback) {
  var self = this;
  var count = this.getSize() + network.genesis.height;
  var i = start || network.genesis.height;
  var lastEntry;
...
ChainDB.prototype.getMaxHeight = function getMaxHeight() {
  var len = this.size / BLOCK_SIZE;
  assert(len % 1 === 0);
  return network.genesis.height + len - 1;
};
...
ChainDB.prototype.getSync = function getSync(height, force) {
  ...
  if (height < network.genesis.height || height == null)
    return;

  var readHeight = height - network.genesis.height;

  if (!force) {
    if ((readHeight + 1) * BLOCK_SIZE > this.size)
      return;
  }
  ...
  data = this._readSync(BLOCK_SIZE, readHeight * BLOCK_SIZE);
...
ChainDB.prototype.getAsync = function getAsync(height, callback, force) {
  ...
  return this._readAsync(BLOCK_SIZE, readHeight * BLOCK_SIZE, function(err, data) {
    var entry;
...
ChainDB.prototype.saveSync = function saveSync(entry) {
  ...
  assert(entry.height >= network.genesis.height);
  var writeHeight = entry.height - network.genesis.height;
  if (writeHeight * BLOCK_SIZE !== this.size) {
    utils.debug('Warning attempt to write to height: %d(%d)/%d',
      writeHeight, entry.height, this.getSize() - 1);
    return false;
  }
  ...
  raw = entry.toRaw();
  offset = writeHeight * BLOCK_SIZE;
  utils.debug('Writing block %s, with offset: %d', entry.height, offset);
...
ChainDB.prototype.saveAsync = function saveAsync(entry, callback) {
  ...
  if(entry.height % 10000 == 0)
    utils.debug('Writing entry height = ' + entry.height);
...
ChainDB.prototype.resetHeightSync = function resetHeightSync(height, emit) {
  ...
  assert(height >= network.genesis.height);
  assert(this.tip);

  var readHeight = height - network.genesis.height;
  size = (readHeight + 1) * BLOCK_SIZE;
  count = this.getSize();

  if ((height - network.genesis.height) > count - 1)
...
ChainDB.prototype.resetHeightAsync = function resetHeightAsync(height, callback, emit) {
  ...
  this.getAsync(height, function(err, tip) {
    ...
    utils.forRange(height + 1, maxHeight + 1, function(i, next) {
      ...
      self._onFlush(function() {
        if (!bcoin.fs) {
          utils.debug('Truncating ramdisk to %i', size);
          self.ramdisk.truncate(size);
          return finish();
        }
