...
function Chain(options) {
  ...
  this.db = new bcoin.chaindb(this, { data: options.data });
...
Chain.prototype._init = function _init() {
  ...
  this._ensureGenesis(function(err) {
    ...
    self._preload(function(err, start) {
      ...
      self.db.load(start || network.genesis.height || 0, function(err) {
        if (err)
          throw err;
...
Chain.prototype._preload = function _preload(callback) {
  ...
  chainHeight = this.db.getMaxHeight();
  height = 0;
  buf = {
    data: [],
    size: 0
  ...
  stream.on('data', function(data) {
    ...
    blocks.forEach(function(data) {
      ...
      if (height === network.genesis.height && entry.hash !== network.genesis.hash) {
        stream.destroy();
...
Chain.prototype._verify = function _verify(block, prev) {
  ...
  if(block.height - network.powDiffInterval >= network.genesis.height) { //FIXME
    if (block.bits !== this.getTarget(prev, block)) {
      utils.debug('Block is using wrong target: %s', block.rhash);
      return false;
    }
  } else {
    utils.debug('Skipping target verification');
  }
...
Chain.prototype.resetHeight = function resetHeight(height, force) {
  assert(height >= network.genesis.height, 'Cannot revert to before %', network.genesis.height);
  var self = this;

  if (height === this.db.getMaxHeight())
    return;
...
Chain.prototype.add = function add(initial, peer, callback, force) {
  ...
  (function next(block) {
    ...
    if (block.getCoinbaseHeight && block.getCoinbaseHeight() > self.bestHeight)
...
Chain.prototype.getLocatorAsync = function getLocatorAsync(start, callback, force) {
  ...
  for (;;) {
    hashes.push(i);
    i = i - step;
    if (i <= network.genesis.height) {
      if (i + step !== network.genesis.height)
        hashes.push(network.genesis.hash);
      break;
    }
    if (hashes.length >= 10)
