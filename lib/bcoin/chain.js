...
function Chain(options) {
  ...
  this.db = this.options.db ? this.options.db(this) : new ChainDB(this);
  this.heightLookup = {};
...
Chain.prototype._init = function _init() {
  if(this.db.constructor.name === 'TestChainDB') {
    var self = this;
    var go = utils.asyncify(function() { 
      self.loading = false;
      utils.debug('TestChainDB...');
      self.emit('load');
    });
    return go();
  }
...
Chain.prototype.getLocator = function getLocator(start) {
  ...
  console.log('locator hashes: ' + JSON.stringify(hashes));
  return hashes;
};
